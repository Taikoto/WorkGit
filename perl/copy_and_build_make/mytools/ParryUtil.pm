...
@EXPORT = qw(set_log_handle set_result_handle showMessage showMessageLn 
            print_to_file open_text_file ltrim rtrim subtrim 
            isLinux isWin32 ClearScreen getSysUserName WordEncode getTimeStr);
...
sub log
{
	...
	if (!defined(Scalar::Util::openhandle($fh)))
	{
...
sub getTimeStr
{
    ...
    return $arrayTime->strftime("%Y.%m.%d %H:%M:%S"); 
}
