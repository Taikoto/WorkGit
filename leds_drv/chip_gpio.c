...
static const struct of_device_id leds[] = {
    { .compatible = "mt2712,leddrv" },
    { },
};
...
static int __init chip_gpio_drv_init(void)
{
    ...
    register_led_operations(&board_led_opr);
    
    return 0;
}

static void __exit chip_gpio_drv_exit(void)
{
    platform_driver_unregister(&chip_gpio_driver);
}

module_init(chip_gpio_drv_init);
module_exit(chip_gpio_drv_exit);
