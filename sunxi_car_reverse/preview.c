...
int g2dScaleRGB(void *src, int srcx, int srcy, int src_width, int src_height,
		void *dst, int dstx, int dsty, int dst_width, int dst_height)
{
#ifdef CONFIG_SUNXI_G2D
	static struct file g2d_mfile;
	g2d_stretchblt scale;
...
int g2dClip(int g2dHandle, void *psrc, int src_w, int src_h, int src_x,
	    int src_y, int width, int height, void *pdst, int dst_w, int dst_h,
	    int dst_x, int dst_y)
{
#ifdef CONFIG_SUNXI_G2D
	g2d_blt blit_para;
...
int g2dRotate(int g2dHandle, g2dRotateAngle angle, unsigned char *src,
	      int src_width, int src_height, unsigned char *dst, int dst_width,
	      int dst_height)
{
#ifdef CONFIG_SUNXI_G2D
	g2d_stretchblt str;
...
void preview_oview_init(int screen_id, int flags)
{
	...
	if (screen_id == 0 && flags) {
		...
		if (aw360_config.size == sizeof(AW360_CONFIG)) {
			memcpy(&ov_configData[screen_id], &aw360_config.birdviewConfig, sizeof(BIRDVIEW_CONFIG));
		...
		for (screen_id = 0; screen_id < num_screens; screen_id++) {
			...
			case V4L2_PIX_FMT_NV21:
				...
				config->info.fb.addr[0] =
				    (unsigned int)disp_frame->phy_address;
...
int preview_output_start(struct preview_params *params)
{
	...
	printk("in allen preview_output_start---\n");
	for (screen_id = 0; screen_id < num_screens; screen_id++) {
		mgr = disp_get_layer_manager(screen_id);
		if (!mgr || !mgr->force_set_layer_config) {
		...
		init_picture();
		alloc_memery();
		alloc_parse_mcu_data_memery();
#ifdef CONFIG_SUPPORT_AUXILIARY_LINE
		printk("in allen before draw_ui input_src = %d car_oview_mode = %d\n",params->input_src,params->car_oview_mode);
...
int clear_cache(int screen_id, void *data,int data_w, int data_h, int p_byte)
{
	...
	printk("screen_id = %d, data_w = %d, data_h = %d, p_byte = %d\r\n",screen_id, data_w, data_h, p_byte);
	//start = data;
	//end = (void *)((unsigned long)start + data_w * data_h * p_byte);
    printk("%s preview[screen_id].auxlayer->vir_address = 0x%x, %x\r\n",__FUNCTION__,preview[screen_id].auxlayer->vir_address,preview[screen_id].auxlayer->size);	
	start1 = preview[screen_id].auxlayer->vir_address;
	end1 = (void *)((unsigned long)start1 + data_w * data_h * p_byte);

	start2 = preview[screen_id].auxlayer_disp->vir_address;
	end2 = (void *)((unsigned long)start2 + data_w * data_h * p_byte);

	memset(preview[screen_id].auxlayer->vir_address, 0, data_w * data_h * p_byte);
	//memset(preview[screen_id].auxlayer_disp->vir_address, 0, data_w * data_h * p_byte);
	dmac_flush_range(start1, end1);
	//dmac_flush_range(start2, end2);
...
int draw_ui(int screen_id, int w, int h)
{
	...
	printk("in allen draw_ui------- screen_id = %d\n",screen_id);
	if (screen_id == 0) {
...
int init_auxiliary_line(int rotate, int screen_id)
{
	...
	printk("preview[screen_id].auxlayer->vir_address = 0x%x\r\n",preview[screen_id].auxlayer->vir_address);
	...
	if (type == 0) {
		...
		*rotate = node;
		...
		blit_para.src_image.addr[0] =
		    (unsigned long)(unsigned int)frame->phy_address;
		...
		blit_para.dst_image.addr[0] =
		    (unsigned long)(unsigned int)node->phy_address;
	...
	for (screen_id = 0; screen_id < num_screens; screen_id++) {
		...
		case V4L2_PIX_FMT_NV21:
			...
			config->info.fb.addr[0] =
			    (unsigned int)frame->phy_address;
			config->info.fb.addr[1] =
			    (unsigned int)frame->phy_address + (width * height);
			config->info.fb.addr[2] = 0;
			config->info.fb.size[0].width = width;
			config->info.fb.size[0].height = height;
			config->info.fb.size[1].width = width / 2;
		...
		case V4L2_PIX_FMT_NV61:
			...
			config->info.fb.crop.height = (unsigned long long)height
						      << 32;
			config->info.screen_win.x = preview[screen_id].frame.x+839;
			config->info.screen_win.y = preview[screen_id].frame.y;
			config->info.screen_win.width =
			    preview[screen_id].frame.w-839;
			config->info.screen_win.height =
			    preview[screen_id].frame.h;
		...
		case V4L2_PIX_FMT_YVU420:
			config->info.fb.format = DISP_FORMAT_YUV420_SP_VUVU;
			config->info.fb.addr[0] =
			    (unsigned int)rotate->phy_address;
			config->info.fb.addr[1] =
			    (unsigned int)rotate->phy_address +
			    (width * height);
			config->info.fb.addr[2] = 0;
		       ...
		       int lr_direct)
{
	...
	for (screen_id = 0; screen_id < num_screens; screen_id++) {
		...
		if (screen_id == 0) {
			...
			dmac_flush_range((void *)srcf[0].addrVirY,
					 (void *)srcf[0].addrVirY +
					     width * height * 3 / 2);
			dmac_flush_range((void *)srcf[1].addrVirY,
					 (void *)srcf[1].addrVirY +
					     width * height * 3 / 2);
			dmac_flush_range((void *)srcf[2].addrVirY,
					 (void *)srcf[2].addrVirY +
					     width * height * 3 / 2);
			dmac_flush_range((void *)srcf[3].addrVirY,
			...
			RenderBVImageMulti(
			    ...
			    width, height, width);
			dmac_flush_range((void *)dstf.addrVirY,
					 (void *)dstf.addrVirY +
					     width * height * 3 / 2);
...
#else
		for (i = 0; i < CAR_MAX_CH; i++) {
			dmac_flush_range((void *)(frame[i]->vir_address),
					 (void *)(frame[i]->vir_address + width * height * 3 / 2));
			merge_frame(frame[i], disp_frame, buffer_format,
				    screen_id, i);
		//	dmac_flush_range((void *)disp_frame->vir_address,
		//			 (void *)disp_frame->vir_address +
		//			     width * height * 3 / 2);
		}
#endif
