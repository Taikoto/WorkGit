...
struct ion_private *__ion_private;
struct ion_private *get_ion_client(void)
{
...
struct buffer_node *__buffer_node_alloc(struct device *dev, int size, int flags)
{
	...
	ION_HEAP_CARVEOUT_MASK|ION_HEAP_TYPE_DMA_MASK, flags ? ION_FLAG_CACHED : 0);
	...
	if (ion_phys(ion->client, mem->handle,
		(ion_phys_addr_t *)&mem->phy_address, &alloc_size)) {
...
void try_release_ion_client(void)
{
...
struct buffer_node *buffer_pool_dequeue(struct buffer_pool *bp)
{
