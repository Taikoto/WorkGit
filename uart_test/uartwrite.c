...
int main(int argc, char **argv) {
	...
	int ret;
	const char tstchr1[] = "123456789012345678901234567890";
	const char tstchr2[] = "abcdefghijklmnopqrstuvwxyzabcd";
	...
	while(i < cnt) {
		...
		if(i%2){
			memcpy(buf,tstchr1,sizeof(tstchr1));
			ret=write(fd,buf,sizeof(tstchr1));
		}else{
			memcpy(buf,tstchr2,sizeof(tstchr2));
			ret=write(fd,buf,sizeof(tstchr2));
		}
		printf("cnt:%d,write(%s) return :%d\r\n",i,argv[PT_DEV_NAME],ret);
		ALOGI("cnt %d : write(%s) return %d.\n",i,argv[PT_DEV_NAME],ret);
		i++;
		sleep(5);
	    str_print(buf,ret);
	}
