...
#endif
#ifdef CONFIG_NAND
#define DEV_TCARD_BLOCK_NAME "/dev/block/mmcblk0"
#else
...
static int eng_linuxcmd_fgucurrent(char *req, char *rsp);
static int eng_linuxcmd_tpfw(char *req, char *rsp);
static int eng_linuxcmd_stopchg(char *req, char *rsp);
static int eng_linuxcmd_enablechg(char *req, char *rsp);
...
static int eng_linuxcmd_get_ap_version(char *req, char *rsp);
...
static struct eng_linuxcmd_str eng_linuxcmd[] = {
    ...
    {CMD_GETFGUCURRENT, CMD_TO_AP, "AT+GETFGUCURRENT", eng_linuxcmd_fgucurrent},
    {CMD_GETTPFW, CMD_TO_AP, "AT+GETTPFW", eng_linuxcmd_tpfw},
    {CMD_STOPCHG, CMD_TO_AP, "AT+STOPCHG", eng_linuxcmd_stopchg},
    {CMD_ENABLECHG, CMD_TO_AP, "AT+ENABLECHG", eng_linuxcmd_enablechg},
...
{CMD_EMMCDDRSIZE,        CMD_TO_AP,     "AT+EMMCDDRSIZE",      eng_linuxcmd_get_emmcddrsize},
    ...
    {CMD_GETAPVER, CMD_TO_AP,  "AT+GETAPVER", eng_linuxcmd_get_ap_version},
};
...
int eng_linuxcmd_fgucurrent(char *req, char *rsp) {
  int fd, ret = 1, len = 0;
  int fgucurrent;
  char *endptr;
  float vol;
  ...
  fd = open(ENG_FGU_CURRENT, O_RDONLY);
  if (fd < 0) {
    ENG_LOG("%s: open %s fail [%s]", __FUNCTION__, ENG_FGU_CURRENT, strerror(errno));
    ret = 0;
  }
  ...
  if (ret == 1) {
    ...
    len = read(fd, buffer, sizeof(buffer));
    if (len > 0) {
      fgucurrent = strtol(buffer, &endptr, 0);
      ENG_LOG("%s: buffer=%s; fgucurrent=%d\n", __FUNCTION__, buffer, fgucurrent);
      sprintf(rsp, "%d%s%s%s", fgucurrent, ENG_STREND, SPRDENG_OK, ENG_STREND);
    } else {
      sprintf(rsp, "%s%s", SPRDENG_ERROR, ENG_STREND);
    }
  } else {
...
int eng_linuxcmd_tpfw(char *req, char *rsp) {
  if (strchr(req, '?') != NULL) {
	  ...
	  fd = open(ENG_CUSTOM_TPFWVER, O_RDONLY);
	  if (fd < 0) {
	    ENG_LOG("%s: open %s fail [%s]", __FUNCTION__, ENG_CUSTOM_TPFWVER, strerror(errno));
	    ret = 0;
	  }
	  ...
	  if (ret == 1) {
	    ...
	    if (len > 0) {
	      ENG_LOG("%s: buffer=%s\n", __FUNCTION__, buffer);
	      sprintf(rsp, "%s%s%s%s", buffer, ENG_STREND, SPRDENG_OK, ENG_STREND);
	    } else {
	      sprintf(rsp, "%s%s", SPRDENG_ERROR, ENG_STREND);
	    }
	  } else {
...
int eng_linuxcmd_stopchg(char *req, char *rsp) {
  ...
  if (fd < 0) {
    ENG_LOG("%s: open %s fail [%s]", __FUNCTION__, ENG_STOPCHG, strerror(errno));
    ret = 0;
  }
...
int eng_linuxcmd_enablechg(char *req, char *rsp) {
  int fd;
  int ret = 1;
  char ok[] = "0";
  fd = open(ENG_STOPCHG, O_WRONLY);
...
static int eng_linuxcmd_get_ap_version(char *req, char *rsp) {
  if (strchr(req, '?') != NULL) {
    char apver[256] = {0};
    property_get(ENG_SPRD_VERS, apver, "");
    sprintf(rsp, "%s", apver);
    return 0;
  } else {
    sprintf(rsp, "%s", "ERROR");
