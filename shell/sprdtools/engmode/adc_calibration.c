...
void initialize_ctrl_file(void) {
  ...
  int ret;
  unsigned int adc_magic;
  char miscdata_path[128] = {0};
  unsigned char *data_buf;
  int data_size = 0;

  data_size = 1024*1024;  //ADC_DATA_START + sizeof(unsigned int) + sizeof(CALI_INFO_DATA_T);
  data_buf = (unsigned char *)malloc(data_size);
  ret = eng_read_productnvdata(data_buf, data_size);
  ENG_LOG(" %s read miscdata size(%d) ret = %d.\n", __func__, data_size, ret);
  if (0 != ret) {
    ENG_LOG("%s: Read before writing ERROR: %d\n", __FUNCTION__, ret);
  } else {
    adc_magic = ADC_MAGIC;
    memcpy(data_buf + ADC_DATA_START, &adc_magic, sizeof(unsigned int));

    cali_info = data_buf + ADC_DATA_START + sizeof(unsigned int);
    if (cali_info->magic != CALI_MAGIC) {
      memset(cali_info, 0xff, sizeof(CALI_INFO_DATA_T));
    ...
    ret = eng_write_productnvdata(data_buf, data_size);
    if (0 != ret) {
