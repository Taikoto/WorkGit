...
#define ENG_PCBA_SUPPORT_CONFIG "/system/etc/PCBA.conf"
#define ENG_BBAT_SUPPORT_CONFIG "/system/etc/BBAT.conf"
...
static int eng_diag_ap_req(char *buf, int len) {
  ...
  if (DIAG_AP_CMD_FILE_OPER == apcmd->cmd) {
    ...
    ret = CMD_USER_TXDATA;
  }else if(DIAG_AP_CMD_TSX_DATA_EXT == apcmd->cmd){
    ret = CMD_USER_TXDATA_EXT;
...
#else
    int y = 0;
    for(y=0; y<sizeof(TOOLS_DIAG_AP_TSX_DATA_T); y++) {
...
#else
    int y = 0;
    for(y=0; y<sizeof(TOOLS_DIAG_AP_TSX_DATA_EXT_T); y++) {
      *((char *)tsxdata + y) = *((char *)src_tsxdata + y);
    }
#endif
