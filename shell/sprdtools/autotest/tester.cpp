...
#define RECORD_MAX_SIZE    1600
    ...
    case 0x01:
            {
            snprintf(write_buf,sizeof(write_buf) -1,"endpoint_test=1;endpoint_teststep=1;test_in_stream_route=0x%x;pcm_rate=16000;pcm_channels=1",input_devices);
            SendAudioTestCmd((const unsigned char *)write_buf,strlen(write_buf));
        }
        break;
    case 0x02:
        ret = 0;
    ...
    case 0x03: {
        ...
        if( data_fd < 0 ) {
		 ...
		 if( 0 == idx ) {
            ...
            unsigned char *buf = (unsigned char *)rcrdBuf;
            int num_read = 0;
            snprintf(write_buf,sizeof(write_buf)-1,"endpoint_test=1;endpoint_teststep=2;pcm_datasize=%d",to_read);
            SendAudioTestCmd((const unsigned char *)write_buf,strlen(write_buf));
            usleep(10000);
            if(data_fd < 0) {
                ALOGD("testAudioIN start open");
                data_fd = open(AUDIO_TEST_FILE, O_RDONLY|O_NONBLOCK);
                ALOGD("testAudioIN end open:%d",data_fd);
            }
            if( data_fd < 0 ) {
            ...
            while(to_read) {
                num_read = read(data_fd, (unsigned char *)buf, to_read);
                if(num_read <= 0) {
                   ALOGW("testAudioIN read:0x%x err:%s", num_read,strerror(errno));
                   usleep(10000);
                    continue;
                }
                if(num_read < to_read) {
                ...
                to_read -= num_read;
                buf += num_read;
                ALOGD("testAudioIN to_read:%d",to_read);
            }
        }
    ...
    break;
    case 0x04:
        snprintf(write_buf, sizeof(write_buf) - 1, "endpoint_test=1;endpoint_teststep=0;");
...
int testAudioOUT(const uchar *data, int data_len, uchar *rsp, int rsp_size)
{
    ...
    switch( act ) {
    case 0x01:{
            printf("testAudioOUT use interpcm :%d start\n",output_devices);
            snprintf(write_buf,
                        sizeof(write_buf)-
                        1,"endpoint_test=2;endpoint_teststep=1;test_stream_route=%d;pcm_rate=44100;pcm_channels=%d;",
                        output_devices,chl);
		ALOGD("write:%s", write_buf);
		SendAudioTestCmd((const unsigned char *)write_buf,strlen(write_buf));
        break;
    ...
    case 0x02: {
        ...
        printf("testAudioOUT :%d start\n",output_devices);
        snprintf(write_buf,
                    sizeof(write_buf)-
                    1,"endpoint_test=2;endpoint_teststep=1;test_stream_route=%d;pcm_rate=44100;pcm_channels=%d;pcm_datasize=%d;pcm_data=%s",
                    output_devices,chl,data_size,str);
		            free(str);
	    ALOGD("write:%s", write_buf);
		SendAudioTestCmd((const unsigned char *)write_buf,strlen(write_buf));
        }
    ...
    case 0x03:
        printf("testAudioOUT :%d stop\n",output_devices);
        snprintf(write_buf, sizeof(write_buf) - 1, "endpoint_test=2;endpoint_teststep=0;");
		ALOGD("write:%s", write_buf);
		SendAudioTestCmd((const unsigned char *)write_buf,strlen(write_buf));
        break;
    default:
