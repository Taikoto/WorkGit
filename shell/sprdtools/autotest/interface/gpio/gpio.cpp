...
int testGpio(char *buf, int len, char *rsp, int rsplen)
{
	...
	***********************************************************************/
	...
	if (gpio_num >= maxnum) {
		LOGD("gpio_num:%d exceed maxnum:%d", gpio_num, maxnum);
		ret = -1;
		goto exit;
	}
	...
	LOGD("testGpio  value  pin_addr = %s!!!gpio_num= %d, maxnum=%d\n",
	...
	system(cmd);
	LOGD("the pad to gpio function = %s\n", cmd);
	usleep(500);

	//add the gpio_num list path
	if(0 != access("/sys/class/gpio/export", F_OK)){
		LOGD("the path can`t find /sys/class/gpio/export node!\n");
		ret = -1;
		goto exit;
	}
