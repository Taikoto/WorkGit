...
extern int ui_wait_key_notimeout();
...
//add the define the tp_dev_patch 
#define  DEV_TP_NAME_PATCH  "/sys/touchscreen/input_name"
#define DEV_EIOT_TP_NAME_PATH "/sys/class/input/input1/name"
...
static int find_input_dev(int mode)
{
	...
	tp_fd = open(DEV_EIOT_TP_NAME_PATH,  O_RDONLY);
	if (tp_fd < 0){
		ERRMSG("open '%s' 	error :%s \n", DEV_EIOT_TP_NAME_PATH, strerror(errno));
		return tp_fd;
	}

	tp_ret = read(tp_fd, tp_name, sizeof(tp_name));
	if(tp_ret < 0){
		ERRMSG("open '%s' 	error :%s \n", DEV_EIOT_TP_NAME_PATH, strerror(errno));
		close(tp_fd);
		return tp_ret;
	...
	while ((de = readdir(dir))) {
		...
		if (fd >= 0) {
			...
			if (ioctl(fd, EVIOCGNAME(sizeof(name) - 1), &name) < 0){
				...
				LOGD("TP-=-name is %s\n", name);
				if (0==strcmp(name, tp_name)) {
					LOGD("name right tp_name = %s\n", name);
...
{
	//LOGD("autotest [%s]:result_has_set=%d,result=%d\n",__func__,result_has_set,result);
	if (result_has_set == 0)
		skd_tp_r = result;
	result_has_set = 1;
	return ;
