...
int test_version_show(void)
{
    ...
    else
    {
        ...
        readsize = read(fd, kernelver, sizeof(kernelver));
        if(readsize > 0)
        {
...
static int getTestsAndResult(int row)
{
	...
	else
	{
		...
		for (i = 0; i < max_station_num && i < 50 && i < SP15_MAX_STATION_NUM; i++)
		{
