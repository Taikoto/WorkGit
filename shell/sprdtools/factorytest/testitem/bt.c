...
{
	unsigned char *addr = (unsigned char *) bdaddr->address;

	snprintf((unsigned char*)bdstr,sizeof(bdstr_t), "%02X:%02X:%02X:%02X:%02X:%02X",
		addr[0],addr[1],addr[2],
		addr[3],addr[4],addr[5]);

	LOGD("mmi test: bt address<%s>",bdstr);
	return (char *)bdstr;
}
...
static int eng_bt_hal_load(void)
{
	...
	rtn = hw_get_module(BT_HARDWARE_MODULE_ID, (hw_module_t const**)&module);

	if (rtn == 0)
	{
		rtn = module->methods->open(module, BT_HARDWARE_MODULE_ID, &device);
		if (rtn == 0)
		{
...
int eng_bt_display(void)
{
	...
	{
		memset(datatmp, 0, sizeof(datatmp));
		strncpy(datatmp, remote_bt_devs[i].bdstr, sizeof(datatmp)-1);
