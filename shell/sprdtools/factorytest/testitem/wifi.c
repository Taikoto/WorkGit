...
static void wifi_show_result()
{
    ...
    for(i = 0; i < sAPNum; i++){
        ...
        snprintf(crow, sizeof(crow), "%s %d",sAPs[i].smac, sAPs[i].frequency);
        ui_show_text(start_y, start_x,crow);
        start_y++;
        memset(crow,0,sizeof(crow));
        snprintf(crow, sizeof(crow), "%d %s",sAPs[i].sig_level,sAPs[i].name);
        ui_show_text(start_y, start_x,crow);
        start_y++;
        if ((start_y + 1) > max_rows){
            break;
        }
    }
    gr_flip();
...
int test_wifi_start(void)
{
	...
	int i=0;
	int ret=0;
	int midrow = max_rows / 2;
	int midcol = gr_fb_width()/CHAR_SIZE/2;
