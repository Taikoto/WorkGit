...
//	at_cmd_audio_loop(0,0,0,0,0,0);
 ...
 void check_headset_by_event(){
	...
	LOGD("mmitest HEK_DEV_NAME = %s; HED_DEV_NAME = %s", SENSOR_DEV_NAME[SENSOR_HDK], SENSOR_DEV_NAME[SENSOR_HDE]);
	if(strlen(SENSOR_DEV_NAME[SENSOR_HDK])){
		fd_key = get_sensor_name(SENSOR_DEV_NAME[SENSOR_HDK]);
		LOGD("++++++++++++++++");
		if(fd_key < 0) {
		...
		LOGD("mmitest get headset keyboard event %s failed",SENSOR_DEV_NAME[SENSOR_HDK]);
	}

	if(strlen(SENSOR_DEV_NAME[SENSOR_HDE])){
		fd_jack = get_sensor_name(SENSOR_DEV_NAME[SENSOR_HDE]);
		LOGD("------------------");
		if(fd_jack < 0) {
			ui_push_result(RL_FAIL);
			ui_set_color(CL_RED);
		...
		LOGD("mmitest get headset jack event %s failed",SENSOR_DEV_NAME[SENSOR_HDE]);
	}
