...
void* modem_init_func()
{
	...
	if(!strcmp(property, "1")){
	    snprintf(modem_count_prop, sizeof(modem_count_prop), PROP_MODEM_LTE_COUNT);
	    property_get(modem_count_prop, modem_prop, "");
	    snprintf(modem_port[0], sizeof(modem_port[0]),"/dev/stty_lte1");
	    snprintf(modem_port[1], sizeof(modem_port[1]),"/dev/stty_lte4");
	}else{
	    snprintf(modem_count_prop, sizeof(modem_count_prop), PROP_MODEM_W_COUNT);
	    property_get(modem_count_prop, modem_prop, "");
	    snprintf(modem_port[0], sizeof(modem_port[0]), "/dev/stty_w1");
	    snprintf(modem_port[1], sizeof(modem_port[1]), "/dev/stty_w4");
	}
