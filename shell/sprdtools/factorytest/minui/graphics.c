...
extern char* freetype(unsigned long, struct Box *);
...
char* gr_text(int x, int y, const char *s)
{
    ...
    while(*s)
    {
        ...
        map = freetype((unsigned long)unicode, &char_position);
        y -= char_position.top;
        x += char_position.left;

        if(x < 0 || y < 0 ||x > gr_fb_width() || y > gr_fb_height())
            LOGD("char position: x = %d, y = %d, width = %d, height = %d", x, y, char_position.width, char_position.height);

        dst_p = gr_draw->data + y * gr_draw->row_bytes + x * gr_draw->pixel_bytes;

        x += char_position.width;
        y += char_position.top;

        if(x >= gr_fb_width())
            return (s-bytes);

        text_blend(map, char_position.width, dst_p, gr_draw->row_bytes, char_position.width, char_position.height);
