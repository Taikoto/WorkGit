...
////ZHOU
#if defined(CONFIG_BIRD_CAR_CRASH_DETECT)
...
#define KEYCODE_WAKEUP   30//116 a
static struct input_dev *kpd_gsensor_dev=NULL;
void irq_config_open(int threshold1);
void irq_close(void);
static int gsensor_kpd_input_init(void)
{
	kpd_gsensor_dev = input_allocate_device();
	if (kpd_gsensor_dev==NULL) 
	{
		printk("[gsensor]kpd_gsensor_dev : fail!\n");
		return -1;
	}
	__set_bit(EV_KEY, kpd_gsensor_dev->evbit);
	__set_bit(KEYCODE_WAKEUP,  kpd_gsensor_dev->keybit);

	kpd_gsensor_dev->id.bustype = BUS_HOST;
	kpd_gsensor_dev->name = "gsensor";
	if(input_register_device(kpd_gsensor_dev))
	{
		printk("[gsensor]kpd_gsensor_dev register : fail!\n");
	}
	else
	{
		printk("[gsensor]kpd_gsensor_dev register : success!!\n");
	} 
	return 1;
}

static int threshold=0xe0;  //默认正负3.5g

extern int mt_gpio_set_debounce(unsigned gpio, unsigned debounce);
extern unsigned int mt_gpio_to_irq(unsigned int gpio);

static struct work_struct sensor_eint_work;
static struct workqueue_struct * sensor_eint_workqueue = NULL;
static int back_irq_number3;

#if defined(CONFIG_BIRD_TBOX_PROJECT)
#define  iddig_pin3        7
#else
#define  iddig_pin3        3
#endif

char cur_gpio3_state=0;
static int back_irq_number3;
void gysensor_eint_callback(struct work_struct *work)
{	


			/************************report key********************/
			input_report_key(kpd_gsensor_dev, KEYCODE_WAKEUP, 1);
			input_sync(kpd_gsensor_dev); 
			input_report_key(kpd_gsensor_dev, KEYCODE_WAKEUP, 0);	
			input_sync(kpd_gsensor_dev);
			/************************report key********************/
    msleep(300);
}

static irqreturn_t back_irq_func0(int irq,void *data)
{
	int ret=0;
#if 0
   if (cur_gpio3_state)
	{
	   cur_gpio3_state=0;	
       irq_set_irq_type(back_irq_number3,IRQ_TYPE_LEVEL_HIGH);	
    }
    else
	{
		cur_gpio3_state=1;
		irq_set_irq_type(back_irq_number3,IRQ_TYPE_LEVEL_LOW);
	} 
#endif	
	ret = queue_work(sensor_eint_workqueue, &sensor_eint_work); 
	return IRQ_HANDLED;
}

static inline int backirq_setup_eint(void)
{
	

    int ret0=0;
    cur_gpio3_state= gpio_get_value(iddig_pin3); 
    gpio_set_debounce(iddig_pin3, 64000);
	back_irq_number3 = mt_gpio_to_irq(iddig_pin3);
	ret0 = request_irq(back_irq_number3, back_irq_func0, IRQF_TRIGGER_FALLING, NULL, NULL);
	if (ret0 > 0)
		printk("back iqr0 not available!!\n");
	else
	{
		//printk("back iqr0 available!!\n");
	}
	return 0; 
}
#endif
		...
		enum BMG_POWERMODE_ENUM power_mode)
{
	...
	#if defined(CONFIG_BIRD_CAR_CRASH_DETECT)
        power_mode=BMG_NORMAL_MODE;//zhou
	#endif
		...
		unsigned long arg)
{
	...
	break;
	#if defined(CONFIG_BIRD_CAR_CRASH_DETECT)
    case GYROSCOPE_IOCTL_INT_CLOSE:
	irq_close();
    break;

    case GYROSCOPE_IOCTL_OPENSET_THRESHOLD:
	data = (void __user *)arg;
    threshold=arg;
    printk("arg=%08x",threshold);
...
#if 0
	//u8 err = 0, data1 = 0x01,data2 = 0x01,data3 = 0x80,data4 = 0x0f,data5 = 0x04,data6 = 0x04;
      u8 err = 0, data1 = 0x01,data2 = 0x01,data3 = 0x80,data4 = 0x0e,data5 = 0x04,data6 = 0x04;   //2.56s
	err += bmg_i2c_write_block(obj_i2c_data->client,0x5f, &data1, 1);

    err += bmg_i2c_write_block(obj_i2c_data->client,0x60, &data2, 1);

    err += bmg_i2c_write_block(obj_i2c_data->client,0x53, &data3, 1);
    err += bmg_i2c_write_block(obj_i2c_data->client,0x54, &data4, 1);
    err += bmg_i2c_write_block(obj_i2c_data->client,0x57, &data5, 1);

    err += bmg_i2c_write_block(obj_i2c_data->client,0x50, &data6, 1);
#endif
//                                                       data3 = 0x80,data4 = 0x0f,data5 = 0x04
    //u8 err = 0, data1 = 0x10,data2 = 0x01,data3 = 0x40,data4 = 0x80,data5 = 0x0e,data6 = 0x02,data7 = 0x04;   //2.56s
      u8 err = 0, data1 = 0x80,data2 = 0x0f,data3 = 0xe0,data4 = 0x80,data5 = 0x0e,data6 = 0x02,data7 = 0x05;   //2.56s
    if((threshold1>=0x10)&&(threshold1<=0xE0)) data3=threshold1;
    printk("threshold=%08x\n",threshold);
    printk("real_thr=%d",data3);
	err = bmg_set_powermode(obj_i2c_data->client,(enum BMG_POWERMODE_ENUM)BMG_NORMAL_MODE);
	err += bmg_i2c_write_block(obj_i2c_data->client,0x5c, &data1, 1);

    err += bmg_i2c_write_block(obj_i2c_data->client,0x5d, &data2, 1);

    err += bmg_i2c_write_block(obj_i2c_data->client,0x5e, &data3, 1);

    err += bmg_i2c_write_block(obj_i2c_data->client,0x53, &data4, 1);
    err += bmg_i2c_write_block(obj_i2c_data->client,0x54, &data5, 1);
    err += bmg_i2c_write_block(obj_i2c_data->client,0x57, &data6, 1);

    err += bmg_i2c_write_block(obj_i2c_data->client,0x51, &data7, 1);///0x05 x  z
...
void irq_close(void)
{
    ...
    u8 err = 0, data1 = 0xfe,data2 = 0,data3 = 0;   //2.56s
   	err = bmg_set_powermode(obj_i2c_data->client,(enum BMG_POWERMODE_ENUM)BMG_NORMAL_MODE);
    err += bmg_i2c_write_block(obj_i2c_data->client,0x5e, &data1, 1);
    err += bmg_i2c_write_block(obj_i2c_data->client,0x51, &data2, 1);///disable irq int 
    err += bmg_i2c_write_block(obj_i2c_data->client,0x53, &data3, 1);
...
static int bmg_i2c_probe(struct i2c_client *client,	const struct i2c_device_id *id)
{
	...
	obj->client->addr = 0x68; //add by Sky [20160708]
...
///ZHOU
#if defined(CONFIG_BIRD_CAR_CRASH_DETECT)
    sensor_eint_workqueue = create_singlethread_workqueue("carcrash_eint0");
	INIT_WORK(&sensor_eint_work, gysensor_eint_callback);
    backirq_setup_eint();
    gsensor_kpd_input_init();
#endif
///ZHOU
	return 0;
...
static int __init bmg_init(void)
{
	...
	i2c_register_board_info(0, &bmg_i2c_info, 1);
